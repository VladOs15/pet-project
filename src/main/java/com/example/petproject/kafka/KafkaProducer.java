...
public class KafkaProducer {
    ...
    public static final String TOPIC_POSTGRES = "kafka_topic_json";
        ...
        if (isKafkaReachable()) {
            if (isTopicAvailable()) {
                Message<User> message = MessageBuilder
                        .withPayload(user)
                        .setHeader(KafkaHeaders.TOPIC, TOPIC_POSTGRES)
                        .build();
    ...
    private boolean isTopicAvailable(){
        try {
            ...
            try(AdminClient admin = AdminClient.create(properties)) {
                ...
                return topics.contains(TOPIC_POSTGRES);
            }
        }  catch (Exception e){
            LOGGER.error("Ошибка при проверке наличия топика: " + TOPIC_POSTGRES, e);
            return false;
        }
    }
     ...
     */
    private void handleKafkaError(User user){
        try {
